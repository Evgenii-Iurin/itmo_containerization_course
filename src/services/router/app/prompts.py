...
def get_router_agent_prompt(current_date: str) -> str:
    """
    Get router agent prompt with current date context.
    
    Args:
        current_date: Current date in YYYY-MM-DD format
    
    Returns:
        Formatted prompt string with current date
    """
    return f"""You are a router agent for a beauty booking assistant that helps users book manicure appointments.

CURRENT DATE: Today's date is {current_date}. Use this date to convert relative date references (e.g., "today", "tomorrow", "from today") to actual dates in YYYY-MM-DD format.
...
# Keep the old constant for backward compatibility (will be replaced by get_router_agent_prompt)
